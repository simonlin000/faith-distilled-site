...
const TODAY_DISMISS_KEY = "faith_distilled_today_dismissed";
...
const elements = {
  ...
  liveUpdateStatus: document.getElementById("live-update-status"),
  ...
  todayDialog: document.getElementById("today-dialog"),
  todayDialogClose: document.getElementById("today-dialog-close"),
  todayDialogSummary: document.getElementById("today-dialog-summary"),
  todayDialogList: document.getElementById("today-dialog-list"),
  todayDialogDismissToggle: document.getElementById("today-dialog-dismiss-toggle"),
...
async function loadJson(path, { fresh = false } = {}) {
  const response = await fetch(fresh ? withCacheBust(path) : path, {
...
function dataSignature(boardsData, watchData, sourcesData) {
  return [
    boardsData?.run_summary?.generated_at || boardsData?.date || "",
    watchData?.generated_at || "",
    sourcesData?.health_summary?.generated_at || "",
    boardsData?.hero?.included_items || 0,
  ].join("|");
}

function setLiveUpdateStatus(text, tone = "neutral") {
  if (!elements.liveUpdateStatus) return;
  elements.liveUpdateStatus.textContent = text;
  elements.liveUpdateStatus.dataset.tone = tone;
}

async function fetchSiteData({ fresh = false } = {}) {
  return Promise.all([
    loadJson("lists/boards.json", { fresh }),
    loadJson("lists/watchlist.json", { fresh }),
    loadJson("lists/sources.json", { fresh }),
  ]);
}
...
function isTodayDismissed() {
  return localStorage.getItem(TODAY_DISMISS_KEY) === state.boardsData?.date;
}

function setTodayDismissed() {
  if (!state.boardsData?.date) return;
  localStorage.setItem(TODAY_DISMISS_KEY, state.boardsData.date);
}

function clearTodayDismissed() {
  localStorage.removeItem(TODAY_DISMISS_KEY);
}
...
function renderHero() {
  ...
  elements.heroMeta.textContent = `${displayDate} 已更新 · ${hero.included_items} 条资讯进入首页 · ${formatGeneratedAt(runSummary.generated_at)}`;
...
function renderTodayDialog() {
  ...
  elements.todayDialogSummary.textContent = `今天新增 ${todayStats.total} 条，其中中文 ${todayStats.zh} 条、国际 ${todayStats.international} 条${todayStats.topSource ? `，最活跃来源是 ${todayStats.topSource}` : ""}。`;
  elements.todayDialogList.innerHTML = todayItems
    .map(
      (item) => `
        <article class="today-dialog-item ${isRead(item.url) ? "is-read" : ""}">
          <div class="today-dialog-date">${item.date}</div>
          <div class="today-dialog-content">
            <a href="${item.url}" data-open-story="${item.url}">${item.title}</a>
            <div class="today-dialog-meta">${item.source} · ${renameTab(item.boardId)}</div>
            <div class="today-dialog-why">${item.why || "这条是今天新进入首页的内容。"}<\/div>
          </div>
        </article>
      `,
    )
...
function maybeShowTodayDialog() {
  if (isTodayDismissed()) return;
  if (!renderTodayDialog()) return;
  elements.todayDialogDismissToggle.checked = false;
  if (!elements.todayDialog.open) {
    elements.todayDialog.showModal();
  }
}
...
function renderSidebar() {
  ...
  setLiveUpdateStatus(`页面每 5 分钟检查一次新数据。最近构建：${formatGeneratedAt(runSummary.generated_at)}。`);
}
...
function reconcilePins() {
  const available = new Set((state.watchData.entities || []).map((item) => item.id));
  state.selectedPins = state.selectedPins.filter((id) => available.has(id));
  if (!state.selectedPins.length) {
    state.selectedPins = state.watchData.default_pins?.filter((id) => available.has(id)) || [];
  }
...
function applySiteData(boardsData, watchData, sourcesData, { initial = false } = {}) {
  state.boardsData = boardsData;
  state.watchData = watchData;
  state.sourcesData = sourcesData;
  state.dataSignature = dataSignature(boardsData, watchData, sourcesData);
  ...
  reconcilePins();
  if (!state.boardsData.tab_order.some((tab) => tab.id === state.activeTab)) {
    state.activeTab = "read";
  }
  render();
}

async function refreshSiteData({ silent = false } = {}) {
  try {
    if (!silent) {
      setLiveUpdateStatus("正在检查最新资讯...", "loading");
    }

    const [boardsData, watchData, sourcesData] = await fetchSiteData({ fresh: true });
    const nextSignature = dataSignature(boardsData, watchData, sourcesData);
    if (nextSignature === state.dataSignature) {
      setLiveUpdateStatus(`没有发现新版本。最近检查：${formatGeneratedAt(new Date().toISOString())}。`);
      return false;
    }

    applySiteData(boardsData, watchData, sourcesData);
    setLiveUpdateStatus(`已自动载入新版本：${formatGeneratedAt(boardsData.run_summary?.generated_at)}。`, "fresh");

    if (state.currentStoryUrl && elements.readingDialog.open) {
      updateReadingNav(state.currentStoryUrl);
    }
    return true;
  } catch (error) {
    console.warn("Live refresh failed", error);
    setLiveUpdateStatus(`自动检查失败：${error.message}`, "warn");
    return false;
  }
}
...
async function boot() {
  try {
    const [boardsData, watchData, sourcesData] = await fetchSiteData();
    applySiteData(boardsData, watchData, sourcesData, { initial: true });
    maybeShowTodayDialog();
    startLiveRefresh();
  } catch (error) {
    console.error(error);
    elements.contentPanels.innerHTML = "";
...
elements.todayDialogClose.addEventListener("click", () => {
  if (elements.todayDialogDismissToggle.checked) {
    setTodayDismissed();
  } else {
    clearTodayDismissed();
  }
  elements.todayDialog.close();
});

elements.todayDialog.addEventListener("close", () => {
  if (elements.todayDialogDismissToggle.checked) {
    setTodayDismissed();
  }
});
